...
expected: handleControlRequest receives a control_request message. For built-in tools (Read, Edit, Write, Bash, Glob, Grep), it writes a control_response with `"deny"` action to the provided writable stream. For mcp\_\_-prefixed tools, it writes a control_response with `"allow"`. Malformed messages are handled gracefully (logged to console.error, returns false).
result: pass
